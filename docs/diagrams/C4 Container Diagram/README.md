...
> Tài liệu diễn giải và sơ đồ kiến trúc trong file này đang chờ vẽ lại và cập nhật **sơ đồ C4 Container** mới theo định vị hệ thống đã thay đổi (baseline hiện có 22 thực thể, gồm `USER_FOLLOW` thuộc FR-59/BR-75 `ACTIVE`; loại bỏ quản lý danh mục động `CATEGORY` / `RECIPE_CATEGORY` và chế độ nấu ăn từng bước `RECIPE_STEP`).  
...
* **Trách nhiệm chính:**
  ...
  * Lưu trữ bền vững dữ liệu nghiệp vụ của **22 thực thể khái niệm trong Conceptual ERD v1.8.0**, gồm `USER_FOLLOW` thuộc phạm vi `ACTIVE`:
    ...
    22. `User Follow` (`USER_FOLLOW`: Quan hệ theo dõi có hướng giữa hai Member theo FR-59/BR-75 `ACTIVE`)
