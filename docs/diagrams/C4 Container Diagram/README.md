...
* **Tên khối:** `User`
* **Loại phân quyền:** `Guest / Member / Administrator`
* **Mô tả:** Người dùng tương tác trực tiếp với hệ thống thông qua trình duyệt web trên máy tính hoặc thiết bị di động để sử dụng các tính năng hỗ trợ lối sống ăn chay:
  * **Khách vãng lai (Guest):** Xem danh sách và chi tiết công thức nấu ăn công khai, tìm kiếm công thức theo danh mục/nguyên liệu, và trải nghiệm AI Chatbot cơ bản dưới sự kiểm soát tốc độ kỹ thuật (`FR-51`, giới hạn tối đa 10 request/phút qua IP + cookie ẩn danh).
  * **Thành viên (Member):** Quản lý hồ sơ cá nhân và thể trạng/dinh dưỡng, đăng tải bài viết công thức cá nhân (`Recipe Post`), bình luận và phản hồi bài viết, lưu công thức yêu thích (`Saved Recipe`), lập kế hoạch bữa ăn cá nhân 7 ngày (`Meal Plan`), tạo danh sách mua sắm (`Shopping List`), nâng cấp và sử dụng các tính năng AI mở rộng theo gói hội viên (FREE, PLUS, PRO).
  * **Quản trị viên (Administrator):** Quản trị danh mục món ăn chuẩn (`Category`), quản lý từ điển nguyên liệu (`Ingredient`), kiểm duyệt nội dung cộng đồng và xử lý báo cáo vi phạm (`Report`), quản lý danh sách tài khoản người dùng và giám sát trạng thái hệ thống.
...
* **Trách nhiệm chính:**
  ...
  * Nhúng trình phát video của YouTube trực tiếp trên trang chi tiết công thức (`FR-20`).
  * Tích hợp Google Identity Services (GIS) button để người dùng kích hoạt luồng đăng nhập nhanh bằng tài khoản Google (`FR-05`).
...
* **Trách nhiệm chính:**
  ...
  * **Thực thi Quy tắc Nghiệp vụ (Business Rules):** Thực thi toàn bộ các ràng buộc kinh doanh (ví dụ: công thức phải có 10–5.000 ký tự hướng dẫn tự do `instructions`, tối đa 1 ảnh đại diện $\le 5$ MB, không có tính năng Like, xử lý kiểm duyệt ghi trực tiếp vào Report).
...
* **Trách nhiệm chính:**
  ...
  * Lưu trữ bền vững dữ liệu nghiệp vụ của đúng **17 thực thể khái niệm chuẩn hóa (Conceptual ERD Baseline)**:
    1. `User` (Tài khoản, hồ sơ cá nhân và chỉ số dinh dưỡng/thể trạng)
    2. `User Ingredient Preference` (Sở thích/dị ứng nguyên liệu)
    3. `Recipe Post` (Bài công thức kèm trường `instructions`, `cover_image_url`, `youtube_url`)
    4. `Category` (Danh mục bài viết/món ăn)
    5. `Recipe Category` (Bảng liên kết N:M Recipe - Category)
    6. `Ingredient` (Từ điển nguyên liệu chuẩn)
    7. `Recipe Ingredient` (Định lượng nguyên liệu theo bài công thức)
    8. `Saved Recipe` (Bộ sưu tập bài viết đã lưu)
    9. `Comment` (Bình luận và phản hồi lồng đa cấp)
    10. `Report` (Báo cáo vi phạm và lý do/kết quả kiểm duyệt)
    11. `Meal Plan` (Kế hoạch bữa ăn 7 ngày)
    12. `Meal Plan Entry` (Chi tiết từng bữa sáng, trưa, tối trong thực đơn)
    13. `Shopping List` (Danh sách mua sắm nguyên liệu)
    14. `Shopping List Item` (Từng món hàng cần mua)
    15. `Subscription` (Gói hội viên: FREE, PLUS, PRO)
    16. `Payment Transaction` (Lịch sử giao dịch thanh toán)
    17. `Notification` (Thông báo người dùng)
  * Lưu trữ siêu dữ liệu (metadata), trạng thái giao dịch, đường dẫn URL trỏ tới ảnh lưu trên Azure Blob Storage (`cover_image_url`), và đường dẫn video YouTube (`youtube_url`).
...
* **Trách nhiệm chính:**
  * Lưu trữ các tệp nhị phân hình ảnh đại diện của bài viết công thức (`Recipe Post`).
  * Giới hạn kỹ thuật nghiêm ngặt: Mỗi bài công thức chỉ có tối đa **1 ảnh bìa** (`cover_image_url`), dung lượng tối đa $\le 5$ MB, định dạng cho phép: JPEG, PNG, WebP.
...
* **Trách nhiệm:**
  * Cung cấp cơ chế đăng nhập một chạm an toàn và tiện lợi cho người dùng thông qua tài khoản Google có sẵn (`FR-05`).
